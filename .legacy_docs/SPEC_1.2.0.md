...
> **v1.1** resolved SPA crawl, AST workers, `storageState`, GraphQL dedupe, and map-header leakage.  
> **v1.2** addresses OOM (disk-backed JS + payload caps), modal click stalls, AST-vs-network path precedence, Babel `RangeError` fallback, and relative-URL page-context permutations. See §21–§22.
...
4. SOURCE MAP DISCOVERY
   ...
   ├─ Read from scratch path (or re-fetch with header policy → write to scratch)
   ├─ Parse trailing sourceMappingURL comment
   ├─ If data: → decode base64/URI JSON
   ├─ Else if relative/absolute URL → fetch map with §8.4 header policy → scratch
   ├─ Else try `{jsUrl}.map` probe (if not already tried)
   └─ On 200 + valid JSON map → reconstruct sources to scratch (paths, not giant in-memory Map)
...
5. AST EXTRACTION (worker pool — never on main thread for large files)
   ...
   │    each job: { id, filePath, url, pageUrls[] } → read file → parse → findings JSON → GC AST
   ├─ Files > --max-js-bytes: skip Babel; regex-fallback only
   ├─ On SyntaxError OR RangeError (stack overflow) → regex-fallback for that file
   └─ Main thread merges finding batches; relative paths left unresolved until MERGE

6. MERGE
   ├─ Expand relative AST paths against each pageUrl in jsUrl→pageUrls (§9.4)
...
3. **Click loop:** for each unique element matching the selector (cap per page, e.g. 40):
...
**Deferred (not v1):** walking `getEventListeners`-style maps, CDP DOMDebugger listener introspection, or robust modal-close heuristics. History observation + broadened selectors + forced clicks cover the majority of React/Vue/Angular router apps.
...
1. Stream/write bytes to `{tmpdir}/js/{sha256(url).slice(0,16)}.js` (or equivalent stable hash).
2. Store only metadata in memory:
...
3. When a JS response is seen, add **current** `page.url()` (post-History-normalized) to `pageUrls`.
4. Reconstructed source-map files likewise land under `{tmpdir}/src/...` with path handles in a metadata map — not a `Map<string, string>` of full sources.
5. AST workers receive `diskPath` (+ `pageUrls` snapshot as `string[]`); they `fs.readFile` locally. OS page cache handles repeated reads; main thread stays lean.
6. On process exit: `fs.rm(tmpdir, { recursive: true })` unless `--keep-tmpdir`.
...
- **Playwright is not a true stream source.** `response.body()` resolves a full `Buffer` in memory; there is no public Node `ReadableStream` from the network layer without raw CDP. Write that Buffer to disk immediately (`fs.promises.writeFile` or a write-stream fed once), drop all references, and let GC reclaim it. The OOM win is still realized because bodies are **not retained** in `PipelineContext` — only transient during the write.
...
**Implementation note (non-normative):** All `--map-header` values are broadcast to **every** `--map-origin` (and to same-origin map fetches only when also appropriate per the table above). v1 does **not** support per-origin header binding (e.g. Sentry key vs Datadog key). Operators targeting multiple map hosts in one run must use credentials safe for all listed origins, or run separate invocations. Document this clearly in the README to avoid accidental token leakage across third-party telemetry providers.
...
// worker → main
{
  ...
  engine: 'ast' | 'regex',
  degradedReason?: 'max-bytes' | 'syntax' | 'range-error' | 'read-error',
}
// or
{
  jobId: string,
...
**Failure isolation:** a worker OOM/crash is restarted; that job falls back to regex on a fresh worker (read file again from disk).
...
4. For each `pageUrl`, `resolvedUrl = new URL(rawPath, pageUrl).href` (WHATWG URL resolution).
5. Emit one finding per distinct `resolvedUrl` (then dedupe). Cap permutations per raw path at e.g. 20 to avoid combinatorial blowups.

Example: `chunk.123.js` contains `fetch('./data')` and was observed on `/dashboard/reports` and `/dashboard/settings` → export both `/dashboard/reports/data` and `/dashboard/settings/data`.

Store `rawPath`, `resolvedUrl`, and `resolvedFromPage` on each finding for provenance.
...
| Relative API paths | Absolute → origin; relative → permute across `jsUrl→pageUrls` (§9.4) |
...
| **P2** | SPA crawl (broad selectors + History observer + **force clicks**) + `-c`/`-H` + `--state` |
| **P3** | Source map discovery + reconstruction to disk + header policy + `--map-header` / `--save-sources` |
| **P4** | Babel AST in **worker pool** (file-path jobs, `RangeError`→regex) + URL normalization |
| **P5** | Merge/dedupe/parameterize + GraphQL keys + **AST path precedence** + **pageUrl permutations** |
...
1. `npx chunkspelunker --help` documents all PRD flags plus `--state`, `--map-header`, `--ast-workers`, `--max-body-bytes`, `--force-clicks`.
...
15. Relative `./data` in a multi-page chunk emits one endpoint per observed page context.
...
| Relative path context | **v1.2:** `jsUrl → Set<pageUrls>`; permute resolutions in Merger |
