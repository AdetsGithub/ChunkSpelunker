...
> **v1.1** — SPA crawl, AST workers, `storageState`, GraphQL dedupe, map-header scoping.  
> **v1.2** — Disk-backed JS cache, force clicks, AST path precedence, `RangeError`→regex, pageUrl permutations.  
> **v1.3** — Auth-synced map fetches, destructive-click exclusions, AST worker timeouts, map-fetch concurrency limits, resolve-then-dedupe relative permutations. See §21–§23.
...
2. **Header hydration from `storageState`:** On INIT, parse `--state` JSON. For each `origins[]` entry whose origin matches the target (or is listed for same-origin policy), scan `localStorage` / `sessionStorage` entries for well-known token keys (case-insensitive):
   ...
   If the value looks like a JWT (`eyJ…`) or a non-empty opaque token, and the operator did **not** already pass `-H Authorization`, synthesize:

   ```text
   Authorization: Bearer <value>
   ```

   Attach this hydrated header to **same-origin** `APIRequestContext` / policy-approved fetches only (never to third-party `--map-origin` hosts — those still use `--map-header` exclusively).

3. **Explicit `-H` wins** over hydration when both are present.
...
5. **Cross-origin maps:** unchanged — `--map-header` / `--map-origin` only; never send hydrated app Bearer tokens off-origin.

Log at `--verbose`: `[.] hydrated Authorization from storageState key=access_token` (do not print the token value).
